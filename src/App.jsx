...
export default function App() {
  ...
  // v8.117 — On bloque l'accès si l'essai est expiré OU si le compte est SUSPENDU
  // (is_active === false). Ce dernier cas couvre la cascade depuis IO CAR : quand
  // un garage est désactivé/archivé (ou son essai fini), la company IOBILL liée
  // passe is_active=false et doit elle aussi être bloquée. On ne bloque que sur
  // `=== false` explicite → les comptes normaux (is_active true/absent) intacts.
  const isSuspended = company.is_active === false;
  if ((isTrialExpired(company) || isSuspended) && !adminBypass) {
    return (
      <TrialExpiredPage
        token={session.token}
        company={company}
